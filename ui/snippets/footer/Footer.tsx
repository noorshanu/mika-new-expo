...
const Footer = () => {
  ...
  const { isLoading, data: linksData } = useQuery<unknown, ResourceError<unknown>, Array<CustomLinksGroup>>(
    ...
    const logoSrc =  useColorModeValue("/static/logo2.svg", "/static/logo.svg");
  ...
  return (
    ...
    >
      <Box flexGrow="1" mb={{ base: 8, lg: 0 }}>
        ...
        <Box mt={{ base: 5, lg: '44px' }}>
          ...
          <Link fontSize="xs" href="https://www.google.com">  <Image src={logoSrc} alt=''   width={165}
        height={165}/></Link>
        </Box>
        <Text mt={ 3 } maxW={{ base: 'unset', lg: '470px' }} fontSize="xs">
       MIKAWA INU is at the forefront of the revolution. With our decentralized approach, we're connecting freelancers and recruiters, leveraging the power of blockchain technology.
        </Text>
      ...
      >
        <Box minW="160px" w={ config.UI.footer.links ? '160px' : '100%' }>
          { config.UI.footer.links && <Text fontWeight={ 500 } mb={ 3 }>MIKAWA INU</Text> }
          <Grid
            gap={ 1 }
    ...
    <Box textAlign={'center'}>
      <Text fontSize={'12px'}>2023© MIKAWA INU LTD | All Rights Reserved
</Text>
    </Box>
    </>
